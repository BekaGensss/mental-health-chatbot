<x-app-layout>
                ...
                <form id="filterForm" method="GET" action="{{ route('admin.data_murid') }}" class="flex flex-col lg:flex-row items-center space-y-4 lg:space-y-0 lg:space-x-4 w-full xl:w-3/5">
                    <div class="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-4 w-full lg:w-auto">
                        ...
                        <div class="relative w-full sm:w-64"> 
                            <select name="class_filter" id="class_filter" class="border-gray-300 rounded-xl shadow-md focus:border-cyan-600 focus:ring-cyan-600 transition duration-200 p-2.5 text-sm font-semibold w-full pr-10 appearance-none bg-white hover:border-cyan-400 cursor-pointer text-gray-800">
                    ...
                    <div class="relative w-full lg:w-64">
                        <input type="text" name="search_query" id="searchInput" placeholder="Cari data murid (Nama atau Kelas...)" value="{{ $searchQuery }}" class="border-gray-300 rounded-xl shadow-md focus:border-cyan-600 focus:ring-cyan-600 transition duration-200 p-2.5 text-sm font-semibold w-full pl-10 pr-12 text-gray-800">
                    ...
                    <div class="grid grid-cols-1 gap-8 mb-12">
                        ...
                        <div class="bg-white p-6 shadow-xl rounded-xl border border-gray-100 transition duration-300 hover:shadow-lg hover:shadow-cyan-300/50 flex flex-col border-l-8 border-cyan-500">
                            <h4 class="text-lg font-extrabold mb-4 text-cyan-700 flex items-center border-b pb-2">
                                <i class="fas fa-spiderweb mr-2 text-cyan-500"></i> Pola Perilaku Keseluruhan Rata-Rata
                            </h4>
                        ...
                        <table id="studentTable" class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kategori DASS-21</th>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status Depresi</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($studentDetails as $student)
                                    @php
                                        ...
                                        [$dassBg, $dassText] = $getBadgeColor($student['dass21_kategori'] ?? 'Normal');
                                        [$gad7Bg, $gad7Text] = $getBadgeColor($student['gad7_kategori']);
                                        [$kpkBg, $kpkText] = $getBadgeColor($student['kpk_global']);

                                        // Logika Penentuan Status Depresi (Contoh: jika salah satu indikator menunjukkan sedang/berat)
                                        $isDepressed = in_array($student['phq9_kategori'], ['Sedang', 'Berat']) || 
                                                       in_array(($student['dass21_kategori'] ?? 'Normal'), ['Sedang', 'Berat']);
                                        $statusLabel = $isDepressed ? 'Terindikasi Depresi' : 'Tidak Terindikasi';
                                        $statusColor = $isDepressed ? 'bg-red-600 text-white' : 'bg-green-600 text-white';
                                    @endphp
                                    <tr class="data-row hover:bg-gray-50 transition duration-150">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full {{ $dassBg }} {{ $dassText }}">
                                                {{ $student['dass21_kategori'] ?? 'Normal' }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <span class="px-4 py-1.5 inline-flex text-xs leading-5 font-extrabold rounded-lg {{ $statusColor }} shadow-sm">
                                                {{ $statusLabel }}
                                            </span>
                                        </td>
                                    </tr>
    ...
    <script>
        ...
        const classColors = classLabels.map((_, i) => i % 2 === 0 ? 'rgba(45, 212, 190, 0.8)' : 'rgba(99, 102, 241, 0.8)');
