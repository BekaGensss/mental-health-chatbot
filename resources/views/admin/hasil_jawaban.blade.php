<x-app-layout>
    ...
    <div class="py-6 sm:py-10 bg-gray-50/70 min-h-screen">
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8 xl:px-16">
            ...
            {{-- Filter, Export, dan Link Formulir (Menggunakan Aksen Cyan) --}}
            <div class="bg-white p-6 shadow-2xl rounded-2xl border border-gray-100 mb-10 flex flex-col lg:flex-row justify-between items-start lg:items-center transition duration-300 hover:shadow-cyan-100/70 space-y-4 lg:space-y-0 lg:space-x-8">

                {{-- Filter Kelas --}}
                <form method="GET" action="{{ route('admin.data_murid') }}" class="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-4 w-full lg:w-auto">
                ...
                <div class="flex items-center space-x-4 w-full lg:w-auto justify-end">
                    ...
                    <a href="http://127.0.0.1:8000/" target="_blank" title="Buka Formulir Murid" class="p-2.5 bg-gray-100 text-gray-600 rounded-xl hover:bg-cyan-100 hover:text-cyan-600 transition duration-300 text-xl shadow-md border border-gray-200">
            ...
            @if(isset($totalMurid) && $totalMurid === 0)
                ...
                <div class="p-12 text-center text-gray-600 border-4 border-dashed border-cyan-300 rounded-xl bg-cyan-50 shadow-xl max-w-4xl mx-auto">
                    ...
                    <p class="text-lg mt-2">Belum ada data pengisian formulir murid {{ $classFilter !== 'all' ? 'untuk Kelas ' . $classFilter : '' }} yang dapat dianalisis saat ini.</p>
                </div>
            @else
            ...
            <div class="bg-white overflow-hidden shadow-2xl rounded-3xl border border-gray-100 p-8 md:p-10 ring-4 ring-cyan-50">
                ...
                {{-- **TATA LETAK BARU: 1 KOLOM (Tumpukan Vertikal) ** --}}
                <div class="grid grid-cols-1 gap-8 mb-12">
                    ...
                    <div class="bg-white p-6 shadow-xl rounded-xl border border-gray-100 transition duration-300 hover:shadow-lg hover:shadow-cyan-300/50 flex flex-col border-l-8 border-cyan-500">
                        ...
                        {{-- Height ditingkatkan untuk chart Radar --}}
                ...
                {{-- Grid untuk 4 chart (1 kolom di mobile, 2 di tablet, 4 di desktop) --}}
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div class="bg-white p-5 shadow-2xl rounded-xl border border-gray-100 hover:shadow-red-100/70 transition duration-300">
                        ...
                        <ul class="space-y-3">
                            @foreach($phq9Summary as $label => $count)
                                @php
                                    // Total harus didasarkan pada $totalMurid jika Anda menampilkan persentase dari SELURUH responden
                    ...
                    <div class="bg-white p-5 shadow-2xl rounded-xl border border-gray-100 hover:shadow-purple-100/70 transition duration-300">
                        ...
                        {{-- DASS-21 memiliki 4 kategori yang berbeda, kita bisa menggunakan grid di dalam list untuk tampilan yang lebih kompak. --}}
    ...
    <script>
        ...
        // DATA PHP / REAL-TIME
        // =======================================================
        // Menggunakan array_values dan array_keys di sini untuk memastikan urutan yang benar
        ...
        // =======================================================
        // Urutan Warna: Minimal/Normal (Blue), Ringan (Light Blue/Indigo), Sedang (Yellow), Berat (Red)
        ...
        const getChartOptions = (chartType, dataArray, index, suggestedMax = null) => {
            const maxVal = Math.max(...(dataArray.length > 0 ? dataArray : [0, 5])); // Tambahkan 5 sebagai fallback untuk maxVal
            const isVertical = index < 4; // Bar Charts (0-3) adalah Vertical
            ...
            return {
                indexAxis: 'x', // Tetap X-axis untuk Bar Chart Vertikal
                responsive: true,
        ...
        const dass21Labels = ['Normal', 'Ringan', 'Sedang', 'Berat']; // Sesuaikan label untuk DASS-21
        new Chart(dass21Ctx, {
            type: 'bar',
            data: {
                labels: dass21Labels,
                datasets: [{
                    label: 'Jumlah Murid',
                    data: dass21Counts, 
                    backgroundColor: classificationColors, // Tetap gunakan urutan warna yang sama
                    borderColor: classificationBorderColors,
                    borderWidth: 1,
                    borderRadius: 6, 
                }]
        ...
        if (lineCtx && weeklyLabels.length > 0) {
            new Chart(lineCtx.getContext('2d'), {
                type: 'line',
                data: {
                    labels: weeklyLabels, // REAL-TIME DATA
                    datasets: [{
                        label: 'Pengisian per Hari',
                        data: weeklySubmissions, // REAL-TIME DATA
                        borderColor: '#10b981', 
        ...
        if (radarCtx && kpkRadarLabels.length > 0) {
            new Chart(radarCtx.getContext('2d'), {
                type: 'radar',
                data: {
                    labels: kpkRadarLabels, // REAL-TIME DATA
                    datasets: [{
                        label: 'Rata-rata Perilaku (Skala 0-5.5)',
                        data: kpkRadarData, // REAL-TIME DATA
                        borderColor: '#6366f1', 
