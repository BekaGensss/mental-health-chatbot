<x-app-layout>
    <x-slot name="header">
        <h2 class="font-bold text-2xl text-gray-800 leading-tight">
            {{ __('Admin Dashboard | Edit Pertanyaan') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white p-8 shadow-xl rounded-xl border border-gray-100">
                ...
                <h3 class="font-extrabold text-3xl mb-6 text-indigo-800 border-b-4 border-indigo-200 pb-2 flex items-center">
                    <i class="fas fa-pencil-alt mr-3 text-indigo-600"></i> Edit Pertanyaan #{{ $question->order }}
                </h3>
                ...
                <form method="POST" action="{{ route('admin.questions.update', $question->id) }}">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="content" id="content" rows="4" required
                            class="mt-1 block w-full border-gray-300 rounded-lg shadow-inner focus:border-indigo-500 focus:ring-indigo-500 p-3 text-base resize-none placeholder-gray-400 transition duration-150">{{ old('content', $question->content) }}</textarea>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8 p-4 bg-indigo-50 rounded-xl border border-indigo-200">
                        <div>
                            ...
                            <select name="type" id="type" required 
                                ...
                                <option value="phq9" @selected(old('type', $question->type) == 'phq9')>PHQ-9 (Depresi)</option>
                                <option value="gad7" @selected(old('type', $question->type) == 'gad7')>GAD-7 (Kecemasan)</option>
                                <option value="kpk" @selected(old('type', $question->type) == 'kpk')>KPK (Perilaku Kesehatan)</option>
                                <option value="dass21" @selected(old('type', $question->type) == 'dass21')>DASS-21 (Depresi, Cemas, Stres)</option>
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" name="order" id="order" value="{{ old('order', $question->order) }}" required min="1"
                    ...
                    @foreach($question->options->sortBy('score') as $index => $option)
                        ...
                        <div class="grid grid-cols-4 gap-4 mb-4 items-center p-4 bg-gray-100 border border-gray-200 rounded-lg">
                            ...
                            <input type="hidden" name="options[{{ $index }}][score]" value="{{ $option->score }}"> {{-- Tambahkan skor sebagai hidden input --}}
                            ...
                            <div class="col-span-1 text-center">
                                ...
                                <input type="number" value="{{ old('options.'.$index.'.score', $option->score) }}" required readonly
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-200 flex justify-between">
                        ...
                        <button type="submit" class="inline-flex items-center px-6 py-2 bg-green-600 border border-transparent rounded-lg font-bold text-sm text-white uppercase tracking-wider shadow-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition duration-300 transform hover:scale-[1.01]">
                            <i class="fas fa-save mr-2"></i> Perbarui Pertanyaan
                        </button>
                    </div>
                </form>
